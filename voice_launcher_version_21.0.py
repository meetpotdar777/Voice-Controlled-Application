...
COMMANDS = {
    ...
    "help": {"type": "assistant_command", "target": "help"}, # New help command
    "what can you do": {"type": "assistant_command", "target": "help"}, # Alias for help
    "list commands": {"type": "assistant_command", "target": "help"}, # Alias for help
    ...
    "open settings": {"type": "open_system_settings", "feedback": "system settings"}, # New
    ...
    # System Power Commands (New)
    "shutdown computer": {"type": "system_power", "action": "shutdown"},
    "restart computer": {"type": "system_power", "action": "restart"},
    "put computer to sleep": {"type": "system_power", "action": "sleep"},
    ...
    "what is my cpu usage": {"type": "info_query", "target": "cpu_usage", "feedback": "CPU usage"}, # New
    "how much ram is used": {"type": "info_query", "target": "ram_usage", "feedback": "RAM usage"}, # New
    "check disk space": {"type": "info_query", "target": "disk_space", "feedback": "disk space"}, # New
    ...
    "search my notes for": {"type": "memory_command", "action": "search"}, # New
    "edit note": {"type": "memory_command", "action": "edit"}, # New
    ...
    "what are my appointments": {"type": "calendar_reminder", "action": "show_reminders"},
    "show reminders for": {"type": "calendar_reminder", "action": "show_reminders_for_day"}, # New
    ...
    "mark reminder complete": {"type": "calendar_reminder", "action": "mark_complete"}, # New
    "mark event complete": {"type": "calendar_reminder", "action": "mark_complete"}, # New
    "set a timer for": {"type": "calendar_reminder", "action": "set_timer"}, # New
    "set an alarm for": {"type": "calendar_reminder", "action": "set_alarm"}, # New
    "show timers": {"type": "calendar_reminder", "action": "show_timers"}, # New
    "show alarms": {"type": "calendar_reminder", "action": "show_alarms"}, # New
    "cancel timer": {"type": "calendar_reminder", "action": "cancel_timer"}, # New
    "cancel alarm": {"type": "calendar_reminder", "action": "cancel_alarm"}, # New
    ...
    "get light status": {"type": "smart_home_control", "action": "get_light_status_specific"}, # New for specific light
...
def listen_command(prompt="Listening...", timeout_seconds=5, phrase_time_limit_seconds=5):
    ...
    Includes a "cancel" keyword for multi-step interactions.
    """
    r = sr.Recognizer()
    ...
    try:
        ...
        if "cancel" in command or "never mind" in command:
            ...
            return "cancel_command" # Special return value for cancellation
        return command
...
def control_system_power(action):
    """Controls system power (shutdown, restart, sleep) with confirmation."""
    current_os = platform.system()
    
    speak(f"Are you sure you want to {action} the computer? Say 'yes' to confirm or 'no' to cancel.")
    confirmation = listen_command(prompt=f"Confirm {action}...")
    if confirmation != "yes":
        speak(f"{action.capitalize()} cancelled.")
        print(f"[System Power] {action.capitalize()} cancelled by user.")
        return
    ...
    try:
        if current_os == "Windows":
            if action == "shutdown":
                ...
                # This command puts the system to sleep. Requires appropriate permissions.
                subprocess.run(['rundll32.exe', 'powrprof.dll,SetSuspendState', '0,1,0'], check=True)
            speak(f"Your computer will {action} shortly.")
            print(f"[System Power] Initiated {action} on Windows.")
        elif current_os == "Linux":
            if action == "shutdown":
                ...
                subprocess.run(['systemctl', 'suspend'], check=True) # Common for systemd
            speak(f"Your computer will {action} shortly.")
            print(f"[System Power] Initiated {action} on Linux.")
        elif current_os == "Darwin": # macOS
            if action == "shutdown":
                subprocess.run(['sudo', 'shutdown', '-h', 'now'], check=True)
            elif action == "restart":
            ...
            speak(f"Your computer will {action} shortly.")
            print(f"[System Power] Initiated {action} on macOS.")
        else:
            speak(f"I cannot {action} your computer on this operating system ({current_os}).")
            print(f"[System Power Error] Unsupported OS for {action}: {current_os}")
    except FileNotFoundError:
        speak(f"System command for {action} not found. Ensure it's in your PATH or you have necessary permissions.")
        print(f"[System Power Error] Command not found for {action} on {current_os}.")
    except subprocess.CalledProcessError as e:
        speak(f"Failed to {action} the computer. You might need to run Jarvis as administrator or grant specific permissions. Error: {e}")
        print(f"[System Power Error] Subprocess error during {action}: {e}")
    except Exception as e:
        speak(f"An unexpected error occurred while trying to {action} your computer: {e}.")
        print(f"[System Power Error] Unexpected error during {action}: {e}")

def open_system_settings():
    """Opens the system settings/preferences."""
    current_os = platform.system()
    try:
        if current_os == "Windows":
            subprocess.run(['start', 'ms-settings:'], shell=True, check=True)
        elif current_os == "Linux":
            # Try common control centers, fallback to xdg-open
            try:
                subprocess.run(['gnome-control-center'], check=True)
            except FileNotFoundError:
                subprocess.run(['xdg-open', 'x-system-settings:'], check=True)
        elif current_os == "Darwin": # macOS
            subprocess.run(['open', '/System/Library/PreferencePanes/'], check=True)
        else:
            speak(f"I cannot open system settings on your operating system ({current_os}).")
            print(f"[System Settings] Unsupported OS for opening settings: {current_os}")
            return
        speak("Opening system settings.")
        print(f"[System Settings] Opened system settings on {current_os}.")
    except FileNotFoundError:
        speak("System command for opening settings not found.")
        print(f"[System Settings Error] Command not found on {current_os}.")
    except subprocess.CalledProcessError as e:
        speak(f"Failed to open system settings. Error: {e}")
        print(f"[System Settings Error] Subprocess error opening settings: {e}")
    except Exception as e:
        speak(f"An unexpected error occurred while trying to open system settings: {e}.")
        print(f"[System Settings Error] Unexpected error opening settings: {e}")
...
def get_system_info(info_type):
    """Fetches and speaks system information (CPU, RAM, Disk)."""
    if info_type == "cpu_usage":
        cpu_percent = psutil.cpu_percent(interval=1) # Measure over 1 second
        speak(f"Current CPU usage is {cpu_percent} percent.")
        print(f"[System Info] CPU Usage: {cpu_percent}%")
    elif info_type == "ram_usage":
        ram = psutil.virtual_memory()
        total_gb = ram.total / (1024**3)
        used_gb = ram.used / (1024**3)
        percent_used = ram.percent
        speak(f"Total RAM is {total_gb:.1f} gigabytes. {used_gb:.1f} gigabytes are currently used, which is {percent_used} percent.")
        print(f"[System Info] RAM Usage: {used_gb:.1f} GB used ({percent_used}%) of {total_gb:.1f} GB total.")
        ...
        speak(f"On this disk, {total_gb:.1f} gigabytes are available in total. {used_gb:.1f} gigabytes are used, and {free_gb:.1f} gigabytes are free. That's {percent_used} percent used.")
        print(f"[System Info] Disk Space: {used_gb:.1f} GB used ({percent_used}%) of {total_gb:.1f} GB total, {free_gb:.1f} GB free.")
    else:
        speak("I can't provide that specific system information.")
        print(f"[System Info Error] Unknown system info type requested: {info_type}")
...
def perform_dynamic_search(user_command_raw, search_engine_type):
    ...
    if not query:
        ...
        if follow_up_query == "cancel_command": return
        if follow_up_query:
...
def add_to_memory(note, category=None):
    ...
    if not category:
        speak(f"What category does this note belong to? For example: idea, task, shopping list, or personal. Say 'cancel' to abort.")
        spoken_category = listen_command(prompt=f"Listening for category...")
        if spoken_category == "cancel_command": return
        if spoken_category:
...
def read_memory(category=None, summarize=False, search_query=None):
    """Reads and speaks the contents of the memory file, optionally by category, summarized, or searched."""
    memory_data = load_memory_data()
    ...
    if category:
        ...
        speak(f"Searching notes for '{search_query}'.")
        for entry in memory_data:
            # Use fuzzy matching for search
            if process.extractOne(search_query.lower(), [entry['note'].lower()])[1] > GLOBAL_CONFIG["FUZZY_MATCH_THRESHOLD"]:
                filtered_notes.append(entry)
        
        if not filtered_notes:
            speak(f"I found no notes matching '{search_query}'.")
            print(f"[Memory Action] No notes found matching '{search_query}'.")
            return
        speak(f"Here are the notes matching '{search_query}':")
        print(f"[Memory Action] Reading notes matching: '{search_query}'.")
    else:
...
def forget_note(note_identifier=None):
    ...
    if not note_identifier:
        speak(f"Which note would you like {GLOBAL_CONFIG['JARVIS_NAME']} to forget? Please tell me the ID number or a keyword from the note. Say 'cancel' to abort.")
        note_identifier = listen_command(prompt="Listening for note ID or keyword...")
        if note_identifier == "cancel_command": return
        if not note_identifier:
    ...
    try:
        ...
        if found_notes:
            speak(f"Are you sure you want to delete note with ID {note_id}: '{found_notes[0]['note']}'? Say 'yes' to confirm or 'no' to cancel.")
            confirmation = listen_command(prompt="Confirm deletion...")
            if confirmation == "yes":
                ...
                print("[Memory Action] Deletion cancelled by user.")
            return
        ...
        if not matching_entries:
            ...
            speak(f"I found one note: '{matching_entries[0]['note']}'. Are you sure you want to delete it? Say 'yes' to confirm or 'no' to cancel.")
            confirmation = listen_command(prompt="Confirm deletion...")
            if confirmation == "yes":
                save_memory_data(updated_memory_data)
                ...
                speak("Deletion cancelled.")
                print("[Memory Action] Deletion cancelled by user.")
                save_memory_data(memory_data) # Revert if cancelled
        else:
            # Multiple matches, ask for clarification
            speak(f"I found multiple notes containing '{note_identifier}'. Please clarify which one you'd like {GLOBAL_CONFIG['JARVIS_NAME']} to forget by saying its ID. Say 'cancel' to abort.")
            ...
            if clarification == "cancel_command":
                speak("Deletion cancelled.")
                print("[Memory Action] Deletion cancelled by user.")
                save_memory_data(memory_data) # Revert if cancelled
                return
            ...
            try:
                ...
                for entry in memory_data: # Iterate original data to ensure correct deletion
                    if entry.get("id") == clarification_id:
                        found_to_delete = entry
                        deleted_one = True
                    else:
                        final_updated_memory.append(entry)
                
                if deleted_one:
                    speak(f"Are you sure you want to delete note with ID {clarification_id}: '{found_to_delete['note']}'? Say 'yes' to confirm or 'no' to cancel.")
                    confirmation = listen_command(prompt="Confirm deletion...")
                    if confirmation == "yes":
                        ...
                        print(f"[Memory Action] Deleted note with ID {clarification_id}: {found_to_delete['note']}")
                    else:
                        speak("Deletion cancelled.")
                        print("[Memory Action] Deletion cancelled by user.")
                        save_memory_data(memory_data) # Revert if cancelled
...
def clear_all_memory():
    ...
    speak(f"Are you sure you want {GLOBAL_CONFIG['JARVIS_NAME']} to clear all your memories? This action cannot be undone. Say 'yes' to confirm or 'no' to cancel.")
...
def edit_note():
    ...
    memory_data = load_memory_data()
    if not memory_data:
        speak(f"{GLOBAL_CONFIG['JARVIS_NAME']} has no notes to edit.")
        return

    speak("Which note would you like to edit? Please tell me the ID number. Say 'cancel' to abort.")
    id_input = listen_command("Listening for note ID...")
    if id_input == "cancel_command": return

    try:
        note_id = int(id_input.strip())
        note_to_edit = None
        for entry in memory_data:
            if entry.get("id") == note_id:
                note_to_edit = entry
        ...
        if note_to_edit:
            speak(f"You want to edit note ID {note_id}: '{note_to_edit['note']}'. What is the new note text? Say 'cancel' to abort.")
            new_note_text = listen_command("Listening for new note text...")
            if new_note_text == "cancel_command": return
            if not new_note_text:
                speak("No new text provided. Aborting edit.")
                return

            speak(f"What is the new category for this note? Current category is '{note_to_edit.get('category', 'uncategorized')}'. Say 'cancel' to abort, or 'same' to keep current.")
            new_category = listen_command("Listening for new category...")
            if new_category == "cancel_command": return
            if new_category.lower() == "same":
                new_category = note_to_edit.get('category', 'uncategorized')
            elif not new_category:
                speak("No new category provided. Keeping current category.")
                new_category = note_to_edit.get('category', 'uncategorized')
            else:
                new_category = new_category.lower().strip()

            note_to_edit['note'] = new_note_text
            note_to_edit['category'] = new_category
            note_to_edit['timestamp'] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") # Update timestamp
            
            save_memory_data(memory_data)
            speak(f"Note ID {note_id} has been updated to: '{new_note_text}' in category '{new_category}'.")
            print(f"[Memory Action] Edited note ID {note_id}.")
        else:
            speak(f"I could not find a note with ID {note_id}.")
            print(f"[Memory Info] No note found with ID {note_id} for editing.")
    except ValueError:
        speak("That was not a valid ID. Please provide a numeric ID.")
        print("[Memory Error] Invalid ID provided for editing.")
    except Exception as e:
        speak(f"An error occurred during note editing: {e}")
        print(f"[Memory Error] Error editing note: {e}")
...
def process_nlp_query(action_type, text_input=None):
    if action_type == "analyze_text":
        ...
        speak("What text would you like me to analyze for sentiment? Say 'cancel' to abort.")
        text_to_analyze = listen_command("Listening for text...")
        if text_to_analyze == "cancel_command": return
        if text_to_analyze:
            analyze_sentiment(text_to_analyze)
        else:
            speak("No text provided for analysis.")
    elif action_type == "summarize_document":
        speak("What document or text should I summarize? Say 'cancel' to abort.")
        document_text = listen_command("Listening for document/text...")
        if document_text == "cancel_command": return
        if document_text:
...
def _load_calendar_data():
    ...
    try:
        with open(calendar_file, "r", encoding="utf-8") as f:
            ...
            for entry in data:
                # Handle 'completed' status if it exists, default to False
                entry['completed'] = entry.get('completed', False)
                entry['triggered'] = entry.get('triggered', False) # New for alarms/timers
...
def _parse_datetime_from_speech(text):
    ...
    elif "yesterday" in text_lower: # Added for flexibility, though usually for past events
        target_date = today - datetime.timedelta(days=1)
    ...
    # Check for specific day of week without "next" (implies current week or next if passed)
    weekdays = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]
    for i, day in enumerate(weekdays):
        if day in text_lower and "next" not in text_lower:
            current_weekday = today.weekday() # Monday is 0, Sunday is 6
            days_diff = i - current_weekday
            if days_diff < 0: # If the day has already passed this week, assume next week
                days_diff += 7
...
def manage_calendar_event(action_type, user_command_raw=None):
    calendar_data = _load_calendar_data()

    if action_type == "add_reminder" or action_type == "add_event":
        speak("What is the reminder or event for? Say 'cancel' to abort.")
        event_text = listen_command("Listening for event text...")
        if event_text == "cancel_command": return
        if not event_text:
            speak("No event text provided. Aborting.")
            return

        speak("When should I add this? For example, 'tomorrow at 3 PM', 'next Monday', or 'today'. Say 'cancel' to abort.")
        time_text = listen_command("Listening for date and time...")
        if time_text == "cancel_command": return
        if not time_text:
        ...
        new_entry = {
            ...
            "triggered": False # New field for alarms/timers
        }
        ...
        upcoming_events = sorted([
            e for e in calendar_data if e['datetime'] >= datetime.datetime.now() and not e['completed'] and e['type'] in ["reminder", "event"]
    ...
    elif action_type == "show_reminders_for_day":
        # Extract the day from the raw command, e.g., "show reminders for tomorrow"
        day_query = user_command_raw.replace("show reminders for", "").strip()
        if not day_query:
            speak("Which day would you like to see reminders for? Say 'cancel' to abort.")
            day_query = listen_command("Listening for day...")
            if day_query == "cancel_command": return

        if not day_query:
            speak("No day provided. Aborting.")
            return

        target_date_obj = _parse_datetime_from_speech(day_query).date() # Get just the date part

        reminders_for_day = sorted([
            e for e in calendar_data 
            if e['datetime'].date() == target_date_obj and not e['completed'] and e['type'] in ["reminder", "event"]
        ], key=lambda x: x['datetime'])

        if not reminders_for_day:
            speak(f"You have no reminders or appointments for {target_date_obj.strftime('%A, %B %d')}.")
            print(f"[Calendar Action] No events found for {target_date_obj}.")
            return

        speak(f"Here are your reminders and appointments for {target_date_obj.strftime('%A, %B %d')}:")
        print(f"\n--- Calendar Entries for {target_date_obj.strftime('%A, %B %d')} ---")
        for event in reminders_for_day:
            event_time_str = event['datetime'].strftime('%I:%M %p')
            speak(f"ID {event['id']}: {event['text']} at {event_time_str}.")
            print(f"ID {event['id']} ({event['type'].capitalize()}): '{event['text']}' at {event_time_str}")
        print("-----------------------------------\n")


    elif action_type == "delete_reminder":
        speak("Which reminder or event would you like to delete? Please tell me the ID number. Say 'cancel' to abort.")
        id_text = listen_command("Listening for ID...")
        if id_text == "cancel_command": return
        try:
            event_id_to_delete = int(id_text.strip())
            
            event_found = False
            for event in calendar_data:
                if event.get('id') == event_id_to_delete:
                    event_found = True
                    break
            
            if event_found:
                speak(f"Are you sure you want to delete the reminder/event with ID {event_id_to_delete}? Say 'yes' to confirm or 'no' to cancel.")
                confirmation = listen_command(prompt="Confirm deletion...")
                if confirmation == "yes":
                    ...
                    speak("Deletion cancelled.")
                    print("[Calendar Info] Deletion cancelled by user.")
            else:
        ...
        speak(f"Are you sure you want {GLOBAL_CONFIG['JARVIS_NAME']} to clear all your reminders and events? This action cannot be undone. Say 'yes' to confirm or 'no' to cancel.")
        confirmation = listen_command(prompt="Say 'yes' to confirm or 'no' to cancel.")
        if "yes" in confirmation:
        ...
        speak("Which reminder or event would you like to mark as complete? Please tell me the ID number. Say 'cancel' to abort.")
        id_text = listen_command("Listening for ID...")
        if id_text == "cancel_command": return
        try:
            event_id_to_complete = int(id_text.strip())
            
            event_found = False
            for event in calendar_data:
                if event.get('id') == event_id_to_complete:
                    if not event['completed']:
                        event['completed'] = True
                        event_found = True
                        break
                    else:
                        speak(f"Reminder/event with ID {event_id_to_complete} is already marked as complete.")
                        print(f"[Calendar Info] Event ID {event_id_to_complete} already complete.")
                        return
            
            if event_found:
                _save_calendar_data(calendar_data)
                speak(f"Reminder or event with ID {event_id_to_delete} has been marked as complete.")
                print(f"[Calendar Action] Marked event with ID: {event_id_to_complete} as complete.")
            else:
                speak(f"I could not find an incomplete reminder or event with ID {event_id_to_complete}.")
                print(f"[Calendar Info] No incomplete event found with ID: {event_id_to_complete}")
        except ValueError:
            speak("That was not a valid ID. Please provide a numeric ID.")
            print(f"[Calendar Error] Invalid ID provided for marking complete: {id_text}")
        except Exception as e:
            speak(f"An error occurred while trying to mark the reminder complete: {e}")
            print(f"[Calendar Error] Error marking reminder complete: {e}")
    
    elif action_type == "set_timer":
        speak("For how many minutes or hours should I set the timer? Say 'cancel' to abort.")
        duration_text = listen_command("Listening for duration...")
        if duration_text == "cancel_command": return
        
        duration_seconds = 0
        if "minute" in duration_text:
            try:
                minutes = int(''.join(filter(str.isdigit, duration_text.split("minute")[0])))
                duration_seconds += minutes * 60
            except ValueError: pass
        if "hour" in duration_text:
            try:
                hours = int(''.join(filter(str.isdigit, duration_text.split("hour")[0])))
                duration_seconds += hours * 3600
            except ValueError: pass
        
        if duration_seconds > 0:
            alarm_time = datetime.datetime.now() + datetime.timedelta(seconds=duration_seconds)
            new_id = 1
            if calendar_data:
                existing_ids = [item.get("id", 0) for item in calendar_data if isinstance(item.get("id"), int)]
                if existing_ids:
            ...
            new_entry = {
                "id": new_id,
                "type": "timer",
                "text": f"Timer for {duration_text}",
                "datetime": alarm_time,
                "completed": False,
                "triggered": False
            }
            calendar_data.append(new_entry)
            _save_calendar_data(calendar_data)
            speak(f"Timer set for {duration_text}. I will alert you at {alarm_time.strftime('%I:%M %p')}.")
            print(f"[Calendar Action] Set timer (ID {new_id}): {duration_text} for {alarm_time}")
        else:
            speak("I couldn't understand the timer duration. Please specify in minutes or hours.")
            print(f"[Calendar Error] Invalid timer duration: {duration_text}")

    elif action_type == "set_alarm":
        speak("What time should I set the alarm for? For example, '7 AM' or '8:30 PM'. Say 'cancel' to abort.")
        alarm_time_text = listen_command("Listening for alarm time...")
        if alarm_time_text == "cancel_command": return

        parsed_alarm_datetime = _parse_datetime_from_speech(alarm_time_text)
        if parsed_alarm_datetime and parsed_alarm_datetime > datetime.datetime.now():
            new_id = 1
            if calendar_data:
                existing_ids = [item.get("id", 0) for item in calendar_data if isinstance(item.get("id"), int)]
                if existing_ids:
            ...
            new_entry = {
                "id": new_id,
                "type": "alarm",
                "text": f"Alarm for {alarm_time_text}",
                "datetime": parsed_alarm_datetime,
                "completed": False,
                "triggered": False
            }
            calendar_data.append(new_entry)
            _save_calendar_data(calendar_data)
            speak(f"Alarm set for {parsed_alarm_datetime.strftime('%I:%M %p on %A, %B %d')}.")
            print(f"[Calendar Action] Set alarm (ID {new_id}): {alarm_time_text} for {parsed_alarm_datetime}")
        else:
            speak("I couldn't understand the alarm time or it's in the past. Please try again.")
            print(f"[Calendar Error] Invalid alarm time: {alarm_time_text}")
    
    elif action_type == "show_timers":
        active_timers = sorted([
            e for e in calendar_data if e['type'] == "timer" and not e['completed'] and not e['triggered']
        ], key=lambda x: x['datetime'])
        
        if not active_timers:
            speak("You have no active timers.")
            print("[Calendar Action] No active timers found.")
            return
        
        speak("Here are your active timers:")
        print("\n--- Active Timers ---")
        for timer in active_timers:
            remaining_time = timer['datetime'] - datetime.datetime.now()
            if remaining_time.total_seconds() > 0:
                mins, secs = divmod(int(remaining_time.total_seconds()), 60)
                hours, mins = divmod(mins, 60)
                time_left_str = ""
                if hours > 0: time_left_str += f"{hours} hours, "
                if mins > 0: time_left_str += f"{mins} minutes, "
                time_left_str += f"{secs} seconds remaining."
                speak(f"ID {timer['id']}: {timer['text']}. {time_left_str}")
                print(f"ID {timer['id']}: '{timer['text']}'. {time_left_str}")
            else:
                speak(f"ID {timer['id']}: {timer['text']} (Expired, but not yet marked complete).")
                print(f"ID {timer['id']}: '{timer['text']}' (Expired).")
        print("---------------------\n")

    elif action_type == "show_alarms":
        active_alarms = sorted([
            e for e in calendar_data if e['type'] == "alarm" and not e['completed'] and not e['triggered']
        ], key=lambda x: x['datetime'])

        if not active_alarms:
            speak("You have no active alarms.")
            print("[Calendar Action] No active alarms found.")
            return
        
        speak("Here are your active alarms:")
        print("\n--- Active Alarms ---")
        for alarm in active_alarms:
            alarm_time_str = alarm['datetime'].strftime('%I:%M %p on %A, %B %d')
            speak(f"ID {alarm['id']}: {alarm['text']}. Set for {alarm_time_str}.")
            print(f"ID {alarm['id']}: '{alarm['text']}'. Set for {alarm_time_str}.")
        print("---------------------\n")

    elif action_type == "cancel_timer" or action_type == "cancel_alarm":
        item_type = "timer" if action_type == "cancel_timer" else "alarm"
        speak(f"Which {item_type} would you like to cancel? Please tell me the ID number. Say 'cancel' to abort.")
        id_text = listen_command("Listening for ID...")
        if id_text == "cancel_command": return
        try:
            item_id_to_cancel = int(id_text.strip())
            
            item_found = False
            updated_calendar_data = []
            for item in calendar_data:
                if item.get('id') == item_id_to_cancel and item['type'] == item_type and not item['completed']:
                    item_found = True
                    speak(f"Are you sure you want to cancel {item_type} with ID {item_id_to_cancel}: '{item['text']}'? Say 'yes' to confirm or 'no' to cancel.")
                    confirmation = listen_command(prompt="Confirm cancellation...")
                    if confirmation == "yes":
                        # Mark as completed/cancelled instead of deleting
                        item['completed'] = True
                        item['triggered'] = True # Also mark as triggered to prevent future alerts
                        updated_calendar_data.append(item)
                        speak(f"{item_type.capitalize()} with ID {item_id_to_cancel} has been cancelled.")
                        print(f"[Calendar Action] Cancelled {item_type} with ID: {item_id_to_cancel}")
                    else:
                        speak("Cancellation cancelled.")
                        print("[Calendar Info] Cancellation cancelled by user.")
                        updated_calendar_data.append(item) # Keep the item if not cancelled
                else:
                    updated_calendar_data.append(item)
            
            _save_calendar_data(updated_calendar_data) # Save the potentially modified list
            if not item_found:
                speak(f"I could not find an active {item_type} with ID {item_id_to_cancel}.")
                print(f"[Calendar Info] No active {item_type} found with ID: {item_id_to_cancel}")
        except ValueError:
            speak("That was not a valid ID. Please provide a numeric ID.")
            print(f"[Calendar Error] Invalid ID provided for cancellation: {id_text}")
        except Exception as e:
            speak(f"An error occurred while trying to cancel the {item_type}: {e}")
            print(f"[Calendar Error] Error cancelling {item_type}: {e}")


# --- Background thread for Alarms/Timers ---
alarm_timer_thread_running = True
def check_alarms_and_timers():
    global alarm_timer_thread_running
    while alarm_timer_thread_running:
        calendar_data = _load_calendar_data()
        updated_data = []
        now = datetime.datetime.now()
        
        for entry in calendar_data:
            if not entry['completed'] and not entry['triggered'] and entry['datetime'] <= now:
                if entry['type'] == "timer":
                    speak(f"Your timer for '{entry['text']}' is complete!")
                    print(f"[Timer Alert] Timer '{entry['text']}' complete.")
                elif entry['type'] == "alarm":
                    speak(f"Alarm! It's {entry['datetime'].strftime('%I:%M %p')}. {entry['text']}")
                    print(f"[Alarm Alert] Alarm '{entry['text']}' triggered.")
                
                entry['triggered'] = True # Mark as triggered to prevent repeated alerts
            updated_data.append(entry)
        
        _save_calendar_data(updated_data) # Save updated status
        time.sleep(5) # Check every 5 seconds

def stop_alarm_timer_thread():
    global alarm_timer_thread_running
    alarm_timer_thread_running = False
    print("[Background Thread] Alarm/Timer checking thread stopped.")
...
SIMULATED_HUE_LIGHTS = {
    ...
    "4": {"name": "Desk Light", "state": {"on": True, "bri": 100, "hue": 30000, "sat": 200}}, # Added more varied state
}
...
def _hue_send_command(method, endpoint, data=None):
    ...
    try:
        ...
        elif "/lights/" in endpoint and method == "GET": # Specific light status
...
def _hue_find_light_id(light_name):
    ...
    light_names_map = {light_data["name"].lower(): light_id for light_id, light_data in SIMULATED_HUE_LIGHTS.items()}
    best_match_name, score = process.extractOne(light_name.lower(), list(light_names_map.keys()))

    if score > GLOBAL_CONFIG["FUZZY_MATCH_THRESHOLD"]: # Use a configurable threshold
        return light_names_map[best_match_name]
    return None
...
def _hue_get_light_status(light_id=None):
    ...
    if light_id:
        ...
        if response and "error" not in response:
            ...
            status_message = f"The {light_name} is currently {status}."
            if status == "on":
                status_message += f" Its brightness is at {int((brightness / 254) * 100)} percent."
                # Add color info if available (simplified)
                if state.get("sat", 0) > 0: # If there's saturation, it's colored
                    speak_color = "a color"
                    # Very basic color approximation based on hue value
                    if 0 <= state.get("hue", 0) < 5000: speak_color = "reddish"
                    elif 5000 <= state.get("hue", 0) < 15000: speak_color = "yellowish"
                    elif 15000 <= state.get("hue", 0) < 25000: speak_color = "greenish"
                    elif 25000 <= state.get("hue", 0) < 35000: speak_color = "cyan"
                    elif 35000 <= state.get("hue", 0) < 45000: speak_color = "blueish"
                    elif 45000 <= state.get("hue", 0) <= 65535: speak_color = "magenta"
                    status_message += f" and it's a {speak_color} color."
            speak(status_message)
            print(f"[Hue Status] {light_name} is {status}, brightness: {brightness}, state: {state}")
        else:
        ...
        if response and "error" not in response:
            ...
            for lid, light_data in response.items():
                ...
                status_message = f"The {name} is {status}"
                if status == "on":
                    status_message += f", at {brightness} percent brightness."
                    if state.get("sat", 0) > 0:
                        speak_color = "a color"
                        if 0 <= state.get("hue", 0) < 5000: speak_color = "reddish"
                        elif 5000 <= state.get("hue", 0) < 15000: speak_color = "yellowish"
                        elif 15000 <= state.get("hue", 0) < 25000: speak_color = "greenish"
                        ...
                        status_message += f" and it's a {speak_color} color."
                speak(status_message)
                print(f"[Hue Status] {name}: {status}, {brightness}% brightness, state: {state}.")
        else:
...
def _hue_set_light(light_id, on=None, brightness=None, color_name=None):
    ...
    if color_name:
        ...
        if color_name_lower == "red":
            ...
            speak(f"I can't set the light to '{color_name}'. Try basic colors like red, blue, green, white, yellow, purple, or orange.")
...
def control_smart_device(action_type, user_command_raw, target_value=None):
    ...
    if action_type == "lights_on":
        if target_value == "all":
            ...
            speak("Which light would you like me to turn on? Say 'cancel' to abort.")
            light_name = listen_command("Listening for light name...")
            if light_name == "cancel_command": return
            if light_name:
        ...
        if target_value == "all":
            ...
            speak("Which light would you like me to turn off? Say 'cancel' to abort.")
            light_name = listen_command("Listening for light name...")
            if light_name == "cancel_command": return
            if light_name:
                light_id = _hue_find_light_id(light_name)
        ...
        # Try to extract light name and percentage from the raw command directly
        light_name = None
        brightness_percent = None

        # Pattern 1: "set [light name] brightness to [X] percent"
        match_phrase = "brightness to"
        if match_phrase in user_command_raw:
            parts = user_command_raw.split(match_phrase)
            if len(parts) == 2:
                light_name_candidate = parts[0].replace("set light", "").replace("set", "").strip()
                num_part = ''.join(filter(str.isdigit, parts[1]))
                if num_part: brightness_percent = int(num_part)
                if light_name_candidate: light_name = light_name_candidate
        
        # If not found by pattern 1, try asking
        if not light_name:
            speak("Which light's brightness would you like to set? Say 'cancel' to abort.")
            light_name = listen_command("Listening for light name...")
            if light_name == "cancel_command": return
        
        if not brightness_percent:
            speak("And to what percentage brightness? Say 'cancel' to abort.")
            brightness_str = listen_command("Listening for brightness percentage...")
            if brightness_str == "cancel_command": return
        ...
        # Example: "set living room light color to red" or "set light to blue"
        light_name = None
        color_name = None

        # Pattern 1: "set [light name] color to [color]"
        match_phrase = "color to"
        if match_phrase in user_command_raw:
            parts = user_command_raw.split(match_phrase)
            if len(parts) == 2:
                light_name_candidate = parts[0].replace("set light", "").replace("set", "").strip()
        ...
        # If not found by pattern 1, try asking
        if not light_name:
            speak("Which light's color would you like to set? Say 'cancel' to abort.")
            light_name = listen_command("Listening for light name...")
            if light_name == "cancel_command": return
        
        if not color_name:
            speak("And to what color? Say 'cancel' to abort.")
            color_name = listen_command("Listening for color...")
            if color_name == "cancel_command": return
        ...
        speak("Which light's status would you like to know, or should I tell you about all lights? Say 'cancel' to abort.")
        status_query = listen_command("Listening for light name or 'all'...")
        if status_query == "cancel_command": return
    ...
    elif action_type == "get_light_status_specific": # Triggered by "get light status"
        speak("Which light's status would you like to know? Say 'cancel' to abort.")
        light_name = listen_command("Listening for light name...")
        if light_name == "cancel_command": return
        if light_name:
            light_id = _hue_find_light_id(light_name)
        ...
        if target_value:
            ...
            speak("What temperature should I set the thermostat to? Say 'cancel' to abort.")
            temp_str = listen_command("Listening for temperature...")
            if temp_str == "cancel_command": return
...
def control_general_music_player(action_type, song_name=None):
    ...
    if action_type == "play_local":
        ...
        if not song_name:
            speak("What song would you like me to play? Say 'cancel' to abort.")
            requested_song = listen_command("Listening for song name...")
            if requested_song == "cancel_command": return
...
def main():
    ...
    # Start the alarm/timer checking thread
    alarm_check_thread = threading.Thread(target=check_alarms_and_timers, daemon=True)
    ...
    try:
        while True:
            ...
            if user_command_raw == "cancel_command": # Handle global cancellation
                continue
            ...
            if matched_command_key:
                ...
                if action_type == "assistant_command":
                    if target == "greet":
                        ...
                        speak(f"I can help with various tasks. Here are some categories of commands: ")
                        # Group commands by type for better help output
                        commands_by_type = {}
                        for cmd_phrase, cmd_info in COMMANDS.items():
                            cmd_type = cmd_info["type"].replace("_", " ").title()
                            if cmd_type not in commands_by_type:
                                commands_by_type[cmd_type] = []
                            commands_by_type[cmd_type].append(cmd_phrase)
                        
                        for cmd_type, phrases in sorted(commands_by_type.items()):
                            if cmd_type == "Hotword Trigger": continue # Skip hotword trigger itself
                            speak(f"For {cmd_type}:")
                            # Speak a few examples, then mention more are in the console
                            examples_to_speak = 3
                            spoken_examples = []
                            for i, phrase in enumerate(phrases):
                                if i < examples_to_speak:
                                    spoken_examples.append(phrase)
                                else:
                                    break
                            
                            if spoken_examples:
                                speak(f"Try commands like: {', '.join(spoken_examples)}.")
                            
                            if len(phrases) > examples_to_speak:
                                print(f"[{GLOBAL_CONFIG['JARVIS_NAME']} Help] More {cmd_type} commands:")
                                for phrase in phrases[examples_to_speak:]:
                                    print(f"  - {phrase}")
                            time.sleep(0.5) # Small pause between categories
                        
                        speak("You can also say 'cancel' at any point during a multi-step command to stop it.")
                ...
                elif action_type == "open_system_settings": # New handler
                    open_system_settings()

                elif action_type == "system_power": # New handler
                    control_system_power(action["action"])

                elif action_type == "volume_control":
                    volume_action = action["action"]
                    if volume_action == "set":
                        try:
                            speak("To what percentage would you like to set the volume? Say 'cancel' to abort.")
                            vol_str = listen_command(prompt="Listening for volume percentage...")
                            if vol_str == "cancel_command": continue
                        ...
                        speak("By how much should I increase the volume? For example, by 10 or 20 percent. Say 'cancel' to abort.")
                        change_str = listen_command(prompt="Listening for volume increase amount...")
                        if change_str == "cancel_command": continue
                        ...
                        speak("By how much should I decrease the volume? For example, by 10 or 20 percent. Say 'cancel' to abort.")
                    ...
                    if target == "time":
                        ...
                        get_system_info("cpu_usage")
                    elif target == "ram_usage":
                        get_system_info("ram_usage")
                    elif target == "disk_space":
                        get_system_info("disk_space")
                    ...
                    if query_for_gemini:
                        ...
                        speak(f"What would you like to ask {GLOBAL_CONFIG['JARVIS_NAME']}? Say 'cancel' to abort.")
                        follow_up_query = listen_command(prompt="Listening for your question...")
                        if follow_up_query == "cancel_command": continue
                        if follow_up_query:
                    ...
                    if memory_action == "add":
                        speak(f"What do you want {GLOBAL_CONFIG['JARVIS_NAME']} to remember? Say 'cancel' to abort.")
                        note_to_add = listen_command(prompt="Listening for your note...")
                        if note_to_add == "cancel_command": continue
                        if note_to_add:
                        ...
                        if not category_to_read: # If no hint, ask the user
                            speak(f"Which category of notes would you like {GLOBAL_CONFIG['JARVIS_NAME']} to read? Say 'cancel' to abort.")
                            category_to_read = listen_command(prompt="Listening for category...")
                            if category_to_read == "cancel_command": continue
                    ...
                    elif memory_action == "search": # New search memory action
                        speak(f"What keyword or phrase would you like to search for in your notes? Say 'cancel' to abort.")
                        search_query = listen_command(prompt="Listening for search query...")
                        if search_query == "cancel_command": continue
                        if search_query:
                            read_memory(search_query=search_query)
                        else:
                            speak("No search query provided. Aborting search.")
                    elif memory_action == "edit": # New edit note action
                        edit_note()
                ...
                # --- NEW FEATURE HANDLERS ---
                    ...
                    # Pass the raw command for specific day parsing
                    manage_calendar_event(calendar_action, user_command_raw=user_command_raw)
        ...
        stop_alarm_timer_thread() # Ensure the background thread is stopped on exit
