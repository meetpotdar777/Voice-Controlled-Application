...
def perform_dynamic_search(user_command_raw, search_engine_type):
    ...
    try:
        ...
        command_end_index = user_command_raw.find(f" {search_engine_type} for")
        if command_end_index == -1: # Handles "find on google [query]"
             command_end_index = user_command_raw.find(f" on {search_engine_type}")

        if command_end_index != -1:
            query = user_command_raw[command_end_index + len(f" {search_engine_type} for") if f" {search_engine_type} for" in user_command_raw else command_end_index + len(f" on {search_engine_type}"):].strip()
        else:
...
def main():
    ...
    while True:
        ...
        if matched_command_key:
            ...
            # Clean up the command for feedback if it's an open/close action
            command_name_for_feedback = user_command_raw.replace("open ", "").replace("close ", "").replace("search ", "").replace("find ", "").strip()
