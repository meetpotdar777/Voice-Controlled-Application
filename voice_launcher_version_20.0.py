...
def manage_calendar_event(action_type, user_command_raw=None):
    ...
    if action_type == "add_reminder" or action_type == "add_event":
        ...
        upcoming_events = sorted([
            e for e in calendar_data if e['datetime'] >= datetime.datetime.now() and not e['completed']
        ...
        reminders_for_day = sorted([
            ...
            if e['datetime'].date() == target_date_obj and not e['completed']
        ], key=lambda x: x['datetime'])
        ...
        try:
            ...
            if event_found:
                _save_calendar_data(calendar_data)
                speak(f"Reminder or event with ID {event_id_to_complete} has been marked as complete.")
            ...
            if action_type == "assistant_command":
                if target == "greet":
                    ...
                    for cmd_data in COMMANDS.values():
                        categories.add(cmd_data["type"].replace("_", " ").title())
                    
                    for i, category in enumerate(sorted(list(categories))):
                        help_message += f"{category}, "
                        if (i + 1) % 3 == 0: # Speak in chunks
                            speak(help_message)
                            help_message = ""
                    if help_message:
                        speak(help_message)
                    speak("You can ask me to open websites or applications, control volume, get weather, manage your memories and calendar, or ask me general questions. Just say a command like 'open google' or 'what is the time'.")
